...
function send_player_spawn(_steam_id, _slot){
    ...
    var _b = buffer_create(5, buffer_fixed, 1); // 1 (identifier for what packet is for) + 2 (x pos) + 2 (y pos)
    buffer_write(_b, buffer_u8, NETWORK_PACKETS.SPAWN_SELF);
    buffer_write(_b, buffer_u16, _pos.x);
    buffer_write(_b, buffer_u16, _pos.y);
    steam_net_packet_send(_steam_id, _b); // tell the guy who joined where to spawn
    buffer_delete(_b);
    server_player_spawn_at_pos(_steam_id, _pos); // tell ourselves where to spawn the guy
    send_other_player_spawn(_steam_id, _pos); // tell everyone else where to spawn the guy
}
...
function send_other_player_spawn(_steam_id, _pos){
    var _b = buffer_create(13, buffer_fixed, 1); // include steam ID so other players know WHO is being spawned
    // 1 (u8 identifier) + 2 + 2 + 8 (steamID: 64 bit ID)
    buffer_write(_b, buffer_u8, NETWORK_PACKETS.SPAWN_OTHER);
    buffer_write(_b, buffer_u16, _pos.x);
    buffer_write(_b, buffer_u16, _pos.y);
    buffer_write(_b, buffer_u64, _steam_id);
    // since we are always player 0 (player with the server object i.e. the host)
    // it can start at _i = 1
